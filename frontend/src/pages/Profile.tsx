...
import { ShoppingBag, MapPin, Heart, Box, CheckCircle, Truck, Package } from 'lucide-react';
import { MOCK_ORDERS, PRODUCTS } from '../data/mockData';

export const Profile: React.FC = () => {
  // Wishlist fallback (items from category bakery)
      ...
      >
        <div className="flex items-center gap-4 flex-col md:flex-row text-center md:text-left">
          <div className="w-20 h-20 rounded-full bg-gradient-to-tr from-primary to-secondary p-1">
            <div className="w-full h-full rounded-full bg-background flex items-center justify-center text-2xl font-black text-white">
              DK
            </div>
          </div>
          <div>
            <h1 className="text-2xl font-black text-white tracking-tight">Devendiran K</h1>
            <p className="text-xs text-textSecondary">Gold Member since 2026</p>
            <span className="inline-block mt-2 text-[10px] px-2 py-0.5 bg-success/15 text-success rounded-full font-bold uppercase tracking-wider">
              Cupertino Local
            </span>
          </div>
        </div>
        ...
        <div className="flex gap-4">
          <div className="glass-panel px-4 py-3 rounded-2xl text-center min-w-[90px]">
            ...
            <span className="text-lg font-bold text-white">{MOCK_ORDERS.length}</span>
          </div>
          <div className="glass-panel px-4 py-3 rounded-2xl text-center min-w-[90px]">
            ...
            {MOCK_ORDERS.map((order) => (
              ...
              >
                <div className="flex justify-between items-center">
                  <div>
                    <span className="text-xs font-bold text-white block font-mono">{order.orderNumber}</span>
                  ...
                  <div className="flex items-center gap-1.5 bg-white/5 px-3 py-1 rounded-xl border border-white/5">
                    ...
                    <span className="text-xs font-bold text-white">{order.status}</span>
                  </div>
                </div>

                {/* Items breakdown list */}
                <div className="space-y-2 pl-1 border-l-2 border-white/5 py-1">
                  {order.items.map((item) => (
                    <div key={item.id} className="flex justify-between text-xs text-textSecondary">
                      <span>
                        {item.productName} <span className="text-white font-semibold">x{item.quantity}</span>
                      </span>
          ...
          <div className="space-y-3">
            ...
            <div className="glass-panel p-4 rounded-2xl border border-white/5 space-y-3">
              <div>
                ...
                <p className="text-xs text-white font-medium mt-0.5">100 Infinity Loop</p>
              ...
              <div>
                ...
                <p className="text-xs text-white font-medium mt-0.5">1 Infinite Loop</p>
                <p className="text-[10px] text-textSecondary">Cupertino, California</p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-3">
              {wishlistItems.map((prod) => (
                <div key={prod.id} className="glass-panel p-3 rounded-2xl border border-white/5 flex items-center gap-3">
                  <img
                    src={prod.imageUrl}
                    alt={prod.name}
